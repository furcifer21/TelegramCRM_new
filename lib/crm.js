...
const transformClient = (client) => {
  if (!client) {
    console.warn('transformClient: client is null or undefined');
    return null;
  }
  try {
    const transformed = {
      ...
      // Поля Telegram-связки (могут отсутствовать в старых записях/схемах)
      telegramUsername: client.telegram_username || client.telegramUsername || null,
      telegramChatId: client.telegram_chat_id || client.telegramChatId || null,
      telegramFirstName: client.telegram_first_name || client.telegramFirstName || null,
      telegramLastName: client.telegram_last_name || client.telegramLastName || null,
...
export const createClient = async (clientData) => {
  ...
  if (!response.data) {
    throw new Error('Клиент создан, но данные не получены');
  }
  const transformed = transformClient(response.data);
  if (!transformed) {
    throw new Error('Ошибка обработки данных клиента');
  }
  return transformed;
};
...
export const getNote = async (id) => {
  const response = await apiGet(`/api/notes/${id}`);
  if (!response.success) {
    if (response.error?.includes('не найдено') || response.error?.includes('404')) {
      return null;
    }
    throw new Error(response.error || 'Ошибка загрузки заметки');
  }
  // Преобразуем названия полей для совместимости
  const note = response.data;
  return {
 ...
 * @returns {Promise<Object|null>} Обновленная заметка или null
 */
export const updateNote = async (id, noteData) => {
  const response = await apiPut(`/api/notes/${id}`, {
    client_id: noteData.clientId || null,
    text: noteData.text || '',
  });
  if (!response.success) {
    ...
    throw new Error(response.error || 'Ошибка обновления заметки');
  }
  // Преобразуем названия полей для совместимости
  const note = response.data;
  return {
 ...
 * @returns {Promise<Object|null>} Напоминание или null если не найдено
 */
export const getReminder = async (id) => {
  const response = await apiGet(`/api/reminders/${id}`);
  if (!response.success) {
    if (response.error?.includes('не найдено') || response.error?.includes('404')) {
      return null;
    }
    throw new Error(response.error || 'Ошибка загрузки напоминания');
  }
  // Преобразуем названия полей для совместимости
