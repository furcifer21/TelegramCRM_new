...
export const getActiveReminders = async () => {
  ...
  return reminders.filter(reminder => {
    // Проверяем, не уведомлено ли и не архивировано ли (для совместимости со старыми данными)
    if (reminder.notified || reminder.archived === true) return false;
...
export const getNonArchivedReminders = async () => {
  const reminders = await getReminders();
  return reminders.filter(reminder => reminder.archived !== true);
};
